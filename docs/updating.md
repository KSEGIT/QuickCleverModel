...
Six settings in `.env` control this. See `.env.example` for each one:
...
| `BONSAI_PRECHECK_TIMEOUT` | 30s | The check before anything changes — asks about now, not about a restart |
